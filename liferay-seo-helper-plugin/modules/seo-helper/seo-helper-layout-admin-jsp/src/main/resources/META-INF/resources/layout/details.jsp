...
<%-- pagegenerator --%>
<% 
List<JournalArticle> articles=JournalArticleServiceUtil.getArticlesByLayoutUuid(selLayout.getGroupId(), selLayout.getUuid());
boolean friendlyUrlFieldDisabled=!articles.isEmpty();
String articleDetailHref="";
for(JournalArticle article:articles){
	PortletURL tempRowURL = PortalUtil.getControlPanelPortletURL(liferayPortletRequest, JournalPortletKeys.JOURNAL, PortletRequest.RENDER_PHASE); // nuova firma
	tempRowURL.setParameter("mvcPath", "/edit_article.jsp");
	tempRowURL.setParameter("redirect", currentURL);
	tempRowURL.setParameter("groupId", String.valueOf(article.getGroupId()));
	tempRowURL.setParameter("folderId", String.valueOf(article.getFolderId()));
	tempRowURL.setParameter("articleId", article.getArticleId());
	tempRowURL.setParameter("status", String.valueOf(article.getStatus()));
	articleDetailHref=tempRowURL.toString();
	articleDetailHref+="#displayPageContent";
}
%>
<%-- pagegenerator --%>
	...
	<c:when test="<%= !group.isLayoutPrototype() %>">
		...
		<c:choose>
			<c:when test="<%= selLayoutType.isURLFriendliable() %>">
				<%-- pagegenerator --%>
				<c:set var="friendlyUrlLabel" value="friendly-url"/>
				<c:set var="friendlyUrlHelpMessage"><%= LanguageUtil.format(request, "for-example-x", "<em>/news</em>", false) %></c:set><%-- LagugageUtil.format new signature --%>
				<c:if test="<%=friendlyUrlFieldDisabled %>">
					<c:set var="friendlyUrlLabel">
						<a href="<%=articleDetailHref%>">
							<liferay-ui:message key="edit-x" arguments="friendly URL"/>
						</a>
					</c:set>
					<c:set var="friendlyUrlHelpMessage" value=""/>
					<% Map<java.util.Locale, java.lang.String> friendlyURLMap = selLayout.getFriendlyURLMap(); %>
					<% Iterator<Locale> localeKeys = friendlyURLMap.keySet().iterator(); %>
					<% while (localeKeys.hasNext()) { %>
					<% 
						Locale curLocale = localeKeys.next();
						String curFriendlyURL = friendlyURLMap.get(curLocale);
						String curLanguageId = LanguageUtil.getLanguageId(curLocale);
					%>
						<aui:input name="<%= HtmlUtil.escapeAttribute(\"friendlyURL\" + StringPool.UNDERLINE + curLanguageId) %>" type="hidden" value="<%=curFriendlyURL  %>" />
					<% } %>
				</c:if>
				...
				<div class="form-group">
					<label for="<portlet:namespace/>friendlyURL">
						${friendlyUrlLabel}
						<liferay-ui:icon-help message='${friendlyUrlHelpMessage}'/>
					</label>
					<div class="input-group liferay-friendly-url-input-group">
						<span class="input-group-addon" id="<portlet:namespace/>urlBase">
							<span class="input-group-constrain">
								<liferay-ui:message key="<%= StringUtil.shorten(friendlyURLBase.toString(), 40) %>" />
							</span>
						</span>
						<liferay-ui:input-localized cssClass="form-control" disabled="<%=friendlyUrlFieldDisabled %>" defaultLanguageId="<%= LocaleUtil.toLanguageId(themeDisplay.getSiteDefaultLocale()) %>" name="friendlyURL" xml="<%= selLayout.getFriendlyURLsXML() %>" />
					</div>
				</div>
				<%-- pagegenerator --%>
			</c:when>
