...
@Component(immediate = true, property = {
	"bundle.symbolic.name=" + BundleConstants.SYMBOLIC_NAME_LAYOUT_ADMIN,
	"servlet.context.name=" + BundleConstants.SERVLET_CONTEXT_NAME_LAYOUT_ADMIN,
	"resource.bundle.base.name=" + BundleConstants.BUNDLE_BASE_NAME
}, service = ResourceBundleLoader.class)
public class LayoutAdminResourceBundleLoader
	extends ResourceBundleLoaderAbstract {

	@Reference(target = "(&(bundle.symbolic.name=" +
		BundleConstants.SYMBOLIC_NAME_LAYOUT_ADMIN + ")" + _EXCLUDE_SELF + ")")
	public void init(
		ResourceBundleLoader resourceBundleLoader) {

		_log.info(
			"Init Intesys resource bundle for " +
				BundleConstants.SYMBOLIC_NAME_LAYOUT_ADMIN);
		super.init(resourceBundleLoader);
	}

	private static final Log _log =
		LogFactoryUtil.getLog(LayoutAdminResourceBundleLoader.class);

	private static final String _EXCLUDE_SELF =
		"(!(component.name=it.intesys.seo.helper.i18n.rbl.LayoutAdminResourceBundleLoader))";
