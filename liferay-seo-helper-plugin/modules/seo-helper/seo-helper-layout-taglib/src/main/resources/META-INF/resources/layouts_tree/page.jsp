...
%>
<%@ include file="./intesys-page.jspf" %>
<%-- from here on in, some variables are different from the original jsp. They are all prefixed with intesys (for example intesysLinkTemplate) --%>
	...
	var treeview = new TreeViewType(
		{
			...
			layouts: <%= intesysLayoutsJson %>,

			<c:if test="<%= Validator.isNotNull(linkTemplate) %>">
				linkTemplate: '<%= HtmlUtil.escapeJS(intesysLinkTemplate) %>',
			</c:if>
			...
			urls: <%= intesysPortletURLsJSONArray.toString() %>
		}
	).render();
